...
# ignore_symbol_list = ['VZ', 'TXN', 'COP', 'ADBE', 'UPS', 'CMCSA', 'AMGN', 'CRM', 'FMX', 'MS', 'TTE', 'PM', 'SCHW', 'HON', 'QCOM', 'RTX', 'TBC', 'TBB', 'RY', 'T', 'IBM', 'DE', 'SAP', 'GS', 'CVS', 'LOW', 'UNP', 'NFLX', 'HDB', 'LMT', 'CAT', 'AMD', 'UL', 'INTC', 'ELV', 'TD', 'HSBC', 'AXP', 'BUD', 'SNY', 'EQNR', 'SPGI', 'SBUX', 'BLK', 'ADP', 'BP', 'GILD', 'INTU', 'BX', 'AACG', 'AAN', 'AAOI', "AAU", "ABCB", "ABEO", "ABUS", "AC", "ACB", "ACCO", "ACEVW", "ACHR", "ACII", "ACMR", "ACR", "ACRX", "ACVA", "ACXP", "ADAG", "ADAP", "ADER", "ADEX", "ADN", "ADTN", "ADV"]
# ignore_symbol_list = list(np.load("E:/Officefield/Analytics/Face scrapping/no_images_symbol.npy"))
ignore_symbol_list = [] 

url1 = 'https://analytics-image.traderverse.io/stocks/executives'
page1 = requests.get(url1).text
soup1 = BeautifulSoup(page1, 'html.parser')
dom1 = etree.HTML(str(soup1))
dirs = dom1.xpath("//td/img[@alt='[DIR]']/parent::td/following-sibling::td/a")
for dir in dirs:
    ignore_symbol_list.append(dir.text.replace("/", ""))
...
def getImages():
    ...
    for symbol in symbols_list:
        if symbol in ignore_symbol_list:
            print(symbol + " Symbol exist")
            continue
        print("Symbol:", symbol)
        start_time = time.time()
        ...
        for i in range(len(df2)):
            ...
            # print(df2["CompanyName"][i], df2["Key_Executive"][i])
            driver.get("https://www.google.com/search?q=" + removeNameTittle(df2["Key_Executive"][i]).replace(" ", "+") + " " + company)
            # driver.find_element_by_xpath("//a[text()='Images']").click()
            image_name = df2["Key_Executive"][i].replace(".", "").replace("  ", " ")
            images_dict[company]["Executives"].append({
                            "Name": df2["Key_Executive"][i].replace("'", "").replace('"', ""),
                            "Images": []
                        })
            try:
                driver.find_element(By.XPATH, "//a[text()='Images']").click()
            except:
                print("No Image section is found...")
                continue
            # image_divs = driver.find_elements_by_xpath("//div[@id='islrg']/div/div")
            try:
                image_divs = driver.find_element(By.XPATH, "//div[@id='islrg']/div/div")
            except:
                print("Connot find any image")
                continue
            # total_images = driver.find_elements_by_xpath("//div[@id='islrg']/div/div")
            total_images = driver.find_elements(By.XPATH, "//div[@id='islrg']/div/div")
            ...
            for j in range(loop_range):
                try:
                    ...
                    driver.find_element(By.XPATH, "//div[@id='islrg']/div/div[" + str(j+1) + "]/a[1]").click()
                except:
                    print("Cannot find image element: " + "//div[@id='islrg']/div/div[" + str(j+1) + "]/a[1]")
                    continue
                # raw_url = driver.find_element_by_xpath("//div[@id='islrg']/div/div[" + str(j+1) + "]/a[1]").get_attribute('href')
                raw_url = driver.find_element(By.XPATH, "//div[@id='islrg']/div/div[" + str(j+1) + "]/a[1]").get_attribute('href')
        ...
        if len(best_images) == 0:
            print("no image found for " + symbol)
            no_image_nyp_list = list(np.load("E:/Officefield/Analytics/Face scrapping/no_images_symbol.npy"))
            no_image_nyp_list.append(symbol)
            np.save("E:/Officefield/Analytics/Face scrapping/no_images_symbol", no_image_nyp_list)
            continue
